...
export async function public_plans(req: Request, res: Response) {
  try {
    ...
    const { data: result, fromCache, cacheBackend } = await cacheResult({
      key: buildCacheKey('travel_plans', 'public', { page, pageSize, ...filters }),
    ...
    res.json(paginatedResponse(result.data, result.total, { page: result.page, pageSize: result.pageSize }));
  } catch (error) {
