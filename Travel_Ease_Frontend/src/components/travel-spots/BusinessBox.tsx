...
export default function BusinessBox({ business }: BusinessBoxProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        const response = await fetch(endpoints.business.reviews(business.id));
        if (response.ok) {
          const data = await response.json();
          // Map backend response to frontend shape
          const mappedReviews: BusinessReview[] = (data.data || []).map((r: any) => ({
            id: r.review_id,
            ...
            user: r.user ? { 
              id: r.user.user_id, 
              name: [r.user.first_name, r.user.last_name].filter(Boolean).join(' ') || 'Anonymous'
            } : null,
          }));
    ...
    fetchReviews();
  }, [showReviews, business.id, reviews.length]);
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg hover:border-gray-300 transition-all duration-200 cursor-pointer">
        ...
        {business.priceRange && (
          <p className="text-sm text-green-600 font-medium mb-3">
            ₱{business.priceRange.min.toLocaleString()} - ₱{business.priceRange.max.toLocaleString()}
          </p>
        )}
