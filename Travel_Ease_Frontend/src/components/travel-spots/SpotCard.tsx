      ...
      >
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-[var(--radius-lg,16px)] bg-gray-100 shrink-0">
          ...
          {formattedRating && (
            <div className="absolute right-3 top-3 flex items-center gap-1 rounded-[var(--radius-md,12px)] bg-white/90 px-2 py-1 shadow-sm backdrop-blur-md">
              <svg className="h-3.5 w-3.5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="text-xs font-semibold text-gray-900">{formattedRating}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1 min-w-0">
          <div className="flex items-start justify-between gap-2 min-w-0">
            <h3 className="text-base font-semibold leading-tight text-gray-900 truncate" title={business.name}>
              {business.name}
            </h3>
          </div>
          
          <div className="flex items-center text-sm text-gray-500 min-w-0">
            <span className="truncate">{business.city || "Location not available"}</span>
          </div>
