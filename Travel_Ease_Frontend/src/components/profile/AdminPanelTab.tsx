...
function TrafficManagement() {
  ...
  const generateSnapshots = useMutation({
    ...
    onSuccess: (data) => {
      setSuccessMsg(
        data.message || "Traffic snapshots successfully generated.",
      );
      setErrorMsg("");
    ...
    onError: (err: any) => {
      setErrorMsg(
        err?.response?.data?.error || "Failed to generate snapshots.",
      );
      setSuccessMsg("");
    },
  });
      ...
      <div className="px-6 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage core system data and external integrations.
          </p>
        </div>
      </div>
      <div className="p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
          <div>
            <h4 className="font-medium text-gray-900">
              Zone Traffic Snapshots
            </h4>
            <p className="text-sm text-gray-500 mt-1">
              Recalculate and cache all OSRM routing metrics between Tagaytay
              zones. Required if map nodes or base traffic multipliers are
              altered.
            </p>
          </div>
          <button
                      ...
                      <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-500">
                        <select
                          ...
                          disabled={
                            ...
                            changeRoleMutation.variables?.id === u.user_id
                          }
                        >
