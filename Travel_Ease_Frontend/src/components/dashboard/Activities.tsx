...
export default function Activities({
  ...
  const [selectedAlternatives, setSelectedAlternatives] = useState<
    any[] | null
  >(null);
  const [triggeringActivityId, setTriggeringActivityId] = useState<
    number | null
  >(null);
  const [addingAlternativeId, setAddingAlternativeId] = useState<number | null>(
    null,
  );
        ...
        {!isLoading && plans.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 font-medium">
              No activities for Day {day_selected}
            </p>
            ...
            return (
              <React.Fragment key={plan.activity_id}>
                ...
                >
                  {/* Index number */}
                  <div
                    className={`absolute -left-2 -top-2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      plan.is_priority
                  ...
                  <div className="pr-10">
                    {/* Location name + traffic */}
                    <div className="flex items-start justify-between gap-3">
                      <div className="w-[70%] max-w-[70%] min-w-0">
                        {plan.location && (
                          <h4 className="font-semibold text-gray-900 break-words pr-1">
                            {plan.location}
                          </h4>
                        )}
                      </div>

                      <div className="w-[30%] max-w-[30%] min-w-[140px]">
                        {index === 0 ? (
                          <TrafficSuggestion
                            originId={null}
                            ...
                            destLat={plan.lat ?? undefined}
                            destLng={plan.lng ?? undefined}
                          ...
                          <TrafficSuggestion
                            ...
                            originLat={plans[index - 1].lat ?? USER_ORIGIN_LAT}
                            originLng={plans[index - 1].lng ?? USER_ORIGIN_LNG}
                            destId={plan.activity_id}
                            destLat={plan.lat ?? undefined}
                            destLng={plan.lng ?? undefined}
                            dayGroup={isWeekend ? "weekend" : "weekday"}
        ...
        <div className="fixed inset-0 z-[1500] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fade-in">
            ...
            <p className="text-gray-500 text-center text-sm mb-6">
              This action cannot be undone. The activity will be permanently
              removed.
            </p>
