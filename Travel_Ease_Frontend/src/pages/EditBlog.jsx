 ...
 * 
 * Usage:
 * 1. Navigate to /blogs/:slug/edit
 * 2. Form is pre-filled with existing blog data
 * 3. Make changes and submit to update
 * 4. Or delete the blog post
 */
export default function EditBlog() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updated = await blogApi.updateBlog(blogId, payload);
      ...
      if (err.response?.status === 400) {
        ...
        setSubmitError("Blog post not found. It may have been deleted.");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccessMessage("Blog post deleted successfully! Redirecting...");
      ...
      if (err.response?.status === 404) {
        setSubmitError("Blog post not found. It may have already been deleted.");
      } else if (err.response?.status === 500) {
        setSubmitError("Server error. Please try again later.");
      } else if (err.code === "ECONNREFUSED" || err.code === "ERR_NETWORK") {
        setSubmitError("Cannot connect to server. Please check your connection.");
      } else {
        setSubmitError("Failed to delete blog post. Please try again.");
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {showDeleteConfirm && (
          <div className="mb-6 p-6 bg-red-50 border-l-4 border-red-500 rounded-lg shadow-sm">
              ...
              <div className="space-y-6">
                ...
                {!saveDraft && (
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Leave empty to update with current date/time
                    </p>
                  </div>
                )}
              </div>
